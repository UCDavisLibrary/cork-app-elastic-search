...
class ElasticSearchModel extends BaseModel {
  ...
  async textSearch(text, options = {}) {
    ...
    body.query.bool.must.push({
      multi_match : {
        query : text,
        fields : this.config.textFields || ['title', 'description']
      }
    });
  ...
  _addFacetsToBody(body) {
    ...
    for( var key in this.config.facets ) {
      if( this.config.facets[key].type === 'facet' ) {
        body.aggs[key] = {
          terms : { 
            field : key,
            size : 1000
      ...
      } else if( this.config.facets[key].type === 'range' ) {
        body.aggs[key+'-min'] = {
          min : { 
            field : key
          }
  ...
  setSort(key, order, exec) {
    this.sort = {key, order};
    if( exec ) this.search(this.getSearch().query);
  }

  setPaging(from = 0, size, exec) {
    this.from = from;
  ...
  async appendFilter(key, value, exec) {
  ...
  async removeFilter(key, value, exec) {
    this.ensurePath('query.bool.filter', []);
    var body = this.getSearch().query;
  ...
  ensurePath(path, last = {}) {
    var object = this.getSearch().query;
    path.split('.')
