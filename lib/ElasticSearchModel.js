...
class ElasticSearchModel extends BaseModel {
  ...
  fromUrlToSearchDocument(urlParts) {
    ...
    while( urlParts.length > 0 ) {
      let part = decode(urlsParts.splice(0, 1)[0]);
  ...
  fromSearchDocumentToUrl(query) {
    ...
    if( query.filters ) {
      for( var attr in query.filters ) {
        ...
        if( Array.isArray(filter.value) ) arr.concat(filter.value);
        else arr.push(filter.value);

        filters.push(JSON.stringify(arr));
      }
    }
  ...
  _parseUrlFilters(txt = '') {
    let filters = {};
    txt.split(',')
        .map(filter => JSON.parse(filter.trim()))
        .forEach(filter => {
          filters[filter[0]] = _parseUrlFilterOp({
            type : _parseUrlFilterType(filter[1]),
            value : _parseUrlFilterValue(filters)
          }, filter[1])
        });
    return filters;
  }
  ...
  _parseUrlFilterValue(filters) {
    ...
    return filter.splice(2, filter.length);
  }
    ...
    if( data.text || data.filters ) {
      ...
      if( data.text ) {
        esBody.query.bool.must = [{
          multi_match : {
            query : data.text,
            fields : this.config.textFields || this.defaultTextFields
          }
        }];
      }
  ...
  getAppSearchDocument() {
    return clone(this.store.data.searchDocument);
  }
  ...
  async defaultSearch() {
    ...
    return this.service.defaultSearch(defaultSearch);
  }
